...
#       load the CNN model to deal with 1D EEG signals
        ...
        optimizer = optim.Adam(my_net.parameters(), lr=lr)  # optim
...
#       test the results
    ...
    net = NeuralNetClassifier(my_net, max_epochs=20)
    kfold = 11
    ...
    grid = GridSearchCV(estimator=net, refit=0,
                        param_grid=params, scoring=scoring, cv=kfold)
    print(x_train.shape)
    print(y_train.shape)
